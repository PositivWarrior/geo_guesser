...
import { CONTINENTS, DEFAULT_TIMER_DURATIONS, ACHIEVEMENT_IDS } from '@/utils/constants';
...
class GameService {
  ...
  async createGameModes(unlockedContinents: string[]): Promise<GameMode[]> {
    const remoteConfig = await this.firebaseService.getRemoteConfigValues();
    const timerDurations = { ...DEFAULT_TIMER_DURATIONS, ...remoteConfig.timerDurations };
    ...
    Object.entries(CONTINENTS).forEach(([continent, data]) => {
      ...
      const isLocked = !unlockedContinents.includes(continent) && continent !== 'Europe';
      ...
      modes.push({
        ...
        timerDuration: timerDurations[continent] || DEFAULT_TIMER_DURATIONS[continent],
      });
    });

    // Create "All World" mode
    const allCountries = this.getAllCountries();
    const allWorldUnlocked = unlockedContinents.includes('All World') || 
    ...
    modes.push({
      ...
      timerDuration: timerDurations['All World'] || DEFAULT_TIMER_DURATIONS['All World'],
    });
  ...
  initializeGame(mode: GameMode): GameState {
    const shuffledCountries = [...mode.countries].sort(() => Math.random() - 0.5);
  ...
  async validateGuess(guess: string, targetCountry: Country): Promise<boolean> {
    ...
    // Check aliases
    const aliasMatch = targetCountry.aliases.some(alias => 
      this.normalizeString(alias) === normalizedGuess
    );
    ...
    if (bestMatch && bestMatch.item.id === targetCountry.id && bestMatch.score! < 0.2) {
      return true;
    }
  ...
  processCorrectGuess(gameState: GameState): GameState {
    ...
    return {
      ...
      currentCountry: remainingCountries.length > 0 ? 
        this.getRandomCountry(remainingCountries) : null,
    };
  }
  ...
  updateTimer(gameState: GameState, deltaTime: number): GameState {
    ...
    return {
      ...
      isGameActive: newTimeRemaining > 0 && gameState.currentCountry !== null,
    };
  }
  ...
  calculateGameResults(gameState: GameState): {
    ...
    const accuracy = (gameState.correctGuesses / gameState.totalCountries) * 100;
    const completionRate = (gameState.correctGuesses / gameState.totalCountries) * 100;
  ...
  async checkAchievements(
    ...
    if (results.completionRate === 100) {
      const achievementId = this.getContinentAchievementId(gameState.currentMode.continent);
      if (achievementId) {
        const achievement = this.createAchievement(achievementId, gameState.currentMode.name);
        unlockedAchievements.push(achievement);
        await this.firebaseService.unlockAchievement(userId, achievement);
      }
    }

    // Check speed achievement for All World
    if (gameState.currentMode.id === 'all_world' && 
      ...
      await this.firebaseService.unlockAchievement(userId, speedAchievement);
    }
    ...
    if (results.accuracy === 100) {
      ...
      await this.firebaseService.unlockAchievement(userId, perfectAchievement);
    }
  ...
  private getCountriesForContinent(continent: string): Country[] {
    ...
    return continentData.countries.map(countryName => 
