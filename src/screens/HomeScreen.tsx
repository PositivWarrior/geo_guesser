...
const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
	...
	const initializeApp = async () => {
		try {
			...
			// Initialize services (skip Firebase on web)
			if (Platform.OS !== 'web') {
				console.log('HomeScreen: Initializing Firebase...');
				await firebaseService.initializeRemoteConfig();
			} else {
				console.log('HomeScreen: Skipping Firebase on web');
			}

			// Get user's unlocked continents (mock for now)
			const unlockedContinents = ['Europe']; // Europe is free
			console.log('HomeScreen: Creating game modes...');

			// Create game modes
			const modes = await gameService.createGameModes(unlockedContinents);
			console.log('HomeScreen: Game modes created:', modes.length);
			setGameModes(modes);
